...
- import는 side-effect → Node 내장 모듈 → 외부 패키지 → 프로젝트 모듈 → 상대 경로 순으로 자동 그룹화합니다.
- side-effect import의 순서는 자동 정렬하지 않습니다. 모듈 초기화 순서에 영향을 주는 변경은 별도로 검토하세요.
- 속성이 2개 이상인 객체는 여러 줄로 작성하고, type 선언 다음의 다른 선언은 빈 줄로 구분합니다. `npm run format`이 관련 Oxlint 안전 수정을 먼저 적용합니다.
