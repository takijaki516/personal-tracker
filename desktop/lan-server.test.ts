...
describe('encrypted local network transport', () => {
  ...
  it('syncs through a real HTTP server, rejects replay and wakes a waiting phone', async () => {
    ...
    const data = { version: 1 as const, days: { '2026-09-15': { ...emptyDay(), weight: 65 } } };
    const doc = editDocument(newDocument('phone'), emptyStore(), data);
    const body = seal({ id: 'one', kind: 'sync', doc, sentAt: Date.now() }, key, randomBytes);
    const response = await fetch(`${url}/sync`, { method: 'POST', body });
    ...
    expect((await fetch(`${url}/sync`, { method: 'POST', body })).status).toBe(400);
    expect(
      (
        await fetch(`${url}/sync`, {
          method: 'POST',
          headers: { Origin: 'https://untrusted.test' },
          body: seal({ id: 'two', kind: 'sync', doc, sentAt: Date.now() }, key, randomBytes),
        })
      ).status,
    ).toBe(403);
    const wait = fetch(`${url}/wait`, {
      method: 'POST',
      body: seal({ id: 'wait', kind: 'wait', sentAt: Date.now() }, key, randomBytes),
    });
    lan.wake();
    ...
    const ack = await fetch(`${url}/ack`, {
      method: 'POST',
      body: seal({ id: 'ack', kind: 'ack', sentAt: Date.now() }, key, randomBytes),
    });
    ...
    expect(
      (
        await fetch(`${url}/wait`, {
          method: 'POST',
          body: seal(
            { id: 'expired', kind: 'wait', sentAt: Date.now() - 600000 },
            key,
            randomBytes,
          ),
        })
