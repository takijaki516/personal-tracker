...
| Oxlint           | 기본 correctness 규칙, 명시적 중괄호, 엄격한 동등 비교, `const` 사용, 중첩 삼항식 금지, 미사용 코드, 명시적 `any` 금지, type import, 중복 import, 객체·선언 줄바꿈, 동적 코드 실행 방지, React key·HTML 삽입·Hooks 규칙 |
...
포맷 기준은 `.oxfmtrc.json`, 린트 기준은 `.oxlintrc.json`에서 관리합니다. import는 side-effect → Node 내장 모듈 → 외부 패키지 → 프로젝트 모듈 → 상대 경로 순으로 그룹화합니다. 실행 순서에 영향을 줄 수 있는 side-effect import는 그룹 안에서 자동 정렬하지 않습니다. 빌드 결과물과 생성된 네이티브 프로젝트는 검사에서 제외하고, `package-lock.json`은 npm이 관리합니다.

린트는 경고도 실패 처리하고 불필요한 disable 주석도 검출합니다. 속성이 2개 이상인 객체는 여러 줄로 작성하고, type 선언 다음에 다른 선언이 오면 빈 줄로 구분합니다. 사용하지 않을 콜백 인자는 `_event`처럼 `_`로 시작하는 이름을 사용하거나 생략하세요. 진단 로그는 `console.warn`·`console.error`를 사용합니다. 규칙 예외가 꼭 필요하다면 적용 범위를 한 줄로 제한하고 이유를 설명하세요.

`npm run format`은 Oxlint의 안전한 자동 수정을 먼저 적용한 뒤 Oxfmt를 실행합니다. 현재 Oxlint는 구문 기반 검사이며, Promise 처리 누락 같은 타입 정보 기반 린트는 활성화하지 않았습니다. TypeScript 검사도 Promise의 오류 처리를 보장하지 않으므로 비동기 작업은 `await` 또는 오류 처리까지 포함한 흐름을 검토하세요.
